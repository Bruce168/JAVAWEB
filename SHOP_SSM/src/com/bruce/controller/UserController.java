...
import com.bruce.utils.UserUtils;
...
public class UserController extends BaseController {
	...
	public String list(Model model) {
		List<User> users = super.getUserService().selectByParams(null);
	...
	public String show(Model model) {
		User user = super.getUserService().selectByPrimaryKey(1);
	...
	public String login(@ModelAttribute @Validated User user,
		...
		if (allErrors != null && allErrors.size() > 0) {
			...
			if (users != null && users.size() > 0) {
				if (users.get(0).getState() == UserUtils.STATE_DESIBLE) {
					model.addAttribute("message", "该用户没有激活！");
				} else {
		...
		if (users != null && users.size() > 0) {
			...
			if (users.get(0) != null) {
				...
				super.getUserService()
						.updateByPrimaryKeySelective(users.get(0));
			} else {
