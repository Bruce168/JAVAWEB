		...
		<div class="span24">
			...
			<table>
				<tbody>
					...
					<c:forEach items="${order.orderItems }" var="ci">
						<tr>
							<td width="60"><input type="hidden" name="id" value="22" />
								<img
								src="${pageContext.request.contextPath }/${ci.product.image}" />
							</td>
							<td><a target="_blank">${ci.product.pname} </a></td>
							<td>￥${ci.product.shopPrice}</td>
							<td class="quantity" width="60"><input type="text"
								name="count" value="${ci.count}" maxlength="4"
								onpaste="return false;" />
								<div>
									<span class="increase">&nbsp;</span> <span class="decrease">&nbsp;</span>
								</div></td>
							<td width="140"><span class="subtotal">￥${ci.subtotal}</span>
							</td>
							<td><a
								href="${ pageContext.request.contextPath }/cart/${ci.product.pid }/deleteCart.do"
			...
			<div class="total">
				<em id="promotion"></em> 商品金额: <strong id="effectivePrice">￥${cart.total
					}</strong>
			</div>
			<sp:form id="orderForm" action="${pageContext.request.contextPath}/order/payOrder.do" method="post" modelAttribute="order" >
				<input type="hidden" name="order.oid" value="${order.oid }" />
				<div class="span24">
					<p>
						收货地址：<input name="addr" type="text" value="${order.addr }"
							style="width:350px" /> <br /> 
						收货人&nbsp;&nbsp;&nbsp;：<input
							name="name" type="text" value="${order.name }"
							style="width:150px" /> <br /> 
						联系方式：<input
							name="phone" type="text" value="${order.phone }" style="width:150px" />
